...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const canvas = document.getElementById('drawingCanvas_900');
        ...
        // Event listeners
        canvas.addEventListener('mousedown', startPan_900);
        canvas.addEventListener('mousemove', pan_900);
        canvas.addEventListener('mouseup', endPan_900);
        canvas.addEventListener('mouseleave', endPan_900);
        canvas.addEventListener('wheel', handleZoom_900);

        // Fungsi untuk menginisialisasi canvas
        window.initCanvas_900 = function() {
            ctx.strokeStyle = 'red';
            ctx.lineWidth = 2;
        ...
        function drawMeasurement_900(startX, startY, endX, endY, distance) {
        ...
        window.drawLine_900 = function() {
            let distance = parseFloat(document.getElementById('distance_900').value);
            let angle = parseFloat(document.getElementById('angle_900').value) || 0;
        ...
        function redrawCanvas_900() {
            ...
            for (let i = 0; i < points.length; i++) {
                drawPoint_900(points[i].x, points[i].y);
                ...
                if (i > 0) {
                    ...
                    drawMeasurement_900(points[i - 1].x, points[i - 1].y, points[i].x, points[i].y, distance
                        .toFixed(2));
                }
            }
        ...
        window.closePolygon_900 = function() {
            if (points.length < 3) {
            ...
            document.querySelector('input[name="luas_bangunan_900[]"]').value = area.toFixed(2);
        ...
        window.clearCanvas_900 = function() {
            if (points.length > 1) {
                points.pop();
                redrawCanvas_900();
            }
...
<script>
    ...
    function getCanvasData() {
        return {
            ...
            tipe_spek: '900' // tambahkan tipe_spek
        };
    }
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if (isset($bangunan->canvas_data) && $bangunan->tipe_spek === '900')
