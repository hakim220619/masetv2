...
<div id="800" style=" margin-top: 20px; display: none;">
    ...
    <div class="mt-3">
        <button type="button" class="btn btn-primary" id="add-lantai-btn_800">
            <i class="fas fa-plus-circle me-2"></i>Tambah Lantai
        </button>
    </div>

    <div id="lantai-container-wrapper_800">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const mainCanvas = document.getElementById('drawingCanvas_800');
        if (mainCanvas) {
            // Set ukuran canvas sesuai dengan container
            resizeCanvas_800(mainCanvas);

            // Inisialisasi canvas utama di sini
            setupCanvas_800('drawingCanvas_800');
            ...
            redrawCanvas_800('drawingCanvas_800');
        }

        // Fungsi untuk menambahkan lantai baru
        const addLantaiBtn = document.getElementById('add-lantai-btn_800');
        if (addLantaiBtn) {
            addLantaiBtn.addEventListener('click', function() {
                        ...
                        <div class="col-md-4">
                            <div class="tools-container">
                                ...
                                <div class="nav-controls mb-3">
                                    <div class="d-flex justify-content-center">
                                        <div class="nav-button-group">
                                            <button type="button" class="nav-btn" onclick="setDirection_800('up', '${uniqueId}')">
                                                <i class="fas fa-chevron-up"></i>
                                            </button>
                                            <div class="d-flex">
                                                <button type="button" class="nav-btn" onclick="setDirection_800('left', '${uniqueId}')">
                                                    <i class="fas fa-chevron-left"></i>
                                                </button>
                                                <button type="button" class="nav-btn" onclick="setDirection_800('down', '${uniqueId}')">
                                                    <i class="fas fa-chevron-down"></i>
                                                </button>
                                                <button type="button" class="nav-btn" onclick="setDirection_800('right', '${uniqueId}')">
                                                    <i class="fas fa-chevron-right"></i>
                                                </button>
                                            </div>
                                        </div>
                                ...
                                <!-- Action Buttons -->
                                <div class="action-buttons mb-4">
                                    <button type="button" class="btn btn-primary w-100 mb-2" onclick="drawLine_800('${uniqueId}')">
                                        <i class="fas fa-pen me-2"></i>Gambar Garis
                                    </button>
                                    <button type="button" class="btn btn-success w-100 mb-2" onclick="closePolygon_800('${uniqueId}')">
                                        <i class="fas fa-vector-square me-2"></i>Tutup Area
                                    </button>
                                    <button type="button" class="btn btn-outline-danger w-100" onclick="clearCanvas_800('${uniqueId}')">
                                        <i class="fas fa-undo me-2"></i>Hapus Garis Terakhir
                                    </button>
                                </div>
                                ...
                                <div class="form-fields">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="hidden" name="faktor_pengali_800[]" value="1" class="pengali-value-${uniqueId}">
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" name="luas_bangunan_800[]" id="luas_${uniqueId}" 
                ...
                const wrapper = document.getElementById('lantai-container-wrapper_800');
                if (wrapper) {
                ...
                setTimeout(() => {
                    ...
                    if (newCanvas) {
                        ...
                        setupCanvas_800(uniqueId);

                        // Gambar canvas awal
                        redrawCanvas_800(uniqueId);
                    }
                }, 100);
    ...
    function resizeCanvas_800(canvas) {
    ...
    function drawPoint_800(x, y, ctx) {
        ctx.beginPath();
    ...
    function drawMeasurement_800(startX, startY, endX, endY, distance, ctx, zoom = 1) {
    ...
    function calculateArea_800(canvasId = 'drawingCanvas_800') {
        let currentPoints;
        let currentScale;
        let currentZoom;

        if (canvasId === 'drawingCanvas_800') {
    ...
    function setupCanvas_800(canvasId) {
        ...
        window.addEventListener('resize', function() {
            resizeCanvas_800(canvas);
            redrawCanvas_800(canvasId);
        });
    }

    // Fungsi untuk menggambar ulang canvas dengan perbaikan
    function redrawCanvas_800(canvasId = 'drawingCanvas_800') {
        const canvas = document.getElementById(canvasId);
        if (!canvas) return;
        ...
        drawGrid_800(ctx, canvas.width, canvas.height, currentZoom, currentOffsetX, currentOffsetY, currentScale);
        ...
        for (let i = 0; i < currentPoints.length; i++) {
            ...
            if (i > 0) {
                ...
                drawMeasurement_800(currentPoints[i - 1].x, currentPoints[i - 1].y,
        ...
        if (isAreaClosed) {
            const area = calculateArea_800(canvasId);
            ...
            const luasInput = canvasId === 'drawingCanvas_800' ?
                document.querySelector('input[name="luas_bangunan_800[]"]') :
    ...
    function drawGrid_800(ctx, width, height, zoom, offsetX, offsetY, gridSize) {
    ...
    function setDirection_800(direction, canvasId = 'drawingCanvas_800') {
        const distanceInput = document.getElementById(canvasId === 'drawingCanvas_800' ? 'distance_800' :
            `distance_${canvasId}`);
        const angleInput = document.getElementById(canvasId === 'drawingCanvas_800' ? 'angle_800' :
            `angle_${canvasId}`);
    ...
    function startPan_800(e, canvasId = 'drawingCanvas_800') {
        const canvas = document.getElementById(canvasId);
        if (!canvas) return;
    ...
    function pan_800(e, canvasId = 'drawingCanvas_800') {
        const canvas = document.getElementById(canvasId);
        if (!canvas) return;
    ...
    function endPan_800(canvasId = 'drawingCanvas_800') {
        const canvas = document.getElementById(canvasId);
        if (!canvas) return;
    ...
    function drawLine_800(canvasId = 'drawingCanvas_800') {
        const distanceInput = document.getElementById(canvasId === 'drawingCanvas_800' ? 'distance_800' :
            `distance_${canvasId}`);
        const angleInput = document.getElementById(canvasId === 'drawingCanvas_800' ? 'angle_800' :
            `angle_${canvasId}`);
    ...
    function closePolygon_800(canvasId = 'drawingCanvas_800') {
        // Dapatkan data untuk canvas yang sesuai
        let currentPoints;
    ...
    function clearCanvas_800(canvasId = 'drawingCanvas_800') {
        // Dapatkan data untuk canvas yang sesuai
        let currentPoints;
        ...
        if (formElements.length > 0) {
            formElements.forEach(form => {
                form.addEventListener('submit', function(e) {
                    ...
                    const canvasDataToSave = getCanvasData_800();
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if (isset($bangunan->canvas_data) && $bangunan->tipe_spek === '200')
            ...
            if (savedData.main) {
                ...
                // Redraw canvas utama
                if (document.getElementById('drawingCanvas_800')) {
                    redrawCanvas_800('drawingCanvas_800');
                    ...
                    if (points.length > 3 &&
                        points[0].x === points[points.length - 1].x &&
                        points[0].y === points[points.length - 1].y) {
                        const area = calculateArea_800('drawingCanvas_800');
                        const luasBangunanInput = document.querySelector('input[name="luas_bangunan_800[]"]');
