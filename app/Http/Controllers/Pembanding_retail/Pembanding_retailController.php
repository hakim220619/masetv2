...
class Pembanding_retailController extends Controller
{
    ...
    {
        $data = PembandingRetail::findOrFail($id);

        // Decode JSON fields
        $data->narasumber = json_decode($data->narasumber, true);

        return view('content.pembanding.edit.edit_retail', compact('data'));
    }

    public function updateRetail(Request $request, $id)
    {
        try {
            $data = PembandingRetail::findOrFail($id);
            $validated = $request->validate([
            ...
            foreach ($fileFields as $field) {
                if ($request->hasFile($field)) {
                    ...
                    // Store new file
                    $validated[$field] = $request->file($field)->store('uploads/foto', 'public');
                }
            }
            ...
            if ($request->has('foto_lainnya')) {
                ...
                $newFoto = $this->handleFotoLainnya($request);
                $validated['foto_lainnya'] = json_encode(array_merge($existingFoto, $newFoto));
            }

            $data->update($validated);

            return redirect()->route('retail.index')->with('success', 'Data retail berhasil diperbarui');
        } catch (\Exception $e) {
            Log::error('Error updating retail: ' . $e->getMessage());
